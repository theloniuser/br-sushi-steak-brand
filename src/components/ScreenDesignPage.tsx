...
/**
 * Get the default screen design name for a section
 * Assumes the screen design is named after the section in PascalCase
 */
function getDefaultScreenDesign(sectionId: string): string {
 // Convert section-id to PascalCase (e.g., "brand-foundation" -> "BrandFoundation")
 return sectionId
 .split('-')
...
export function ScreenDesignPage() {
 ...
 return (
 <div className="h-screen bg-stone-100 animate-fade-in flex flex-col overflow-hidden">
 {/* Header */}
 <header className="border-b border-stone-200 bg-white shrink-0 z-50">
 <div className="px-4 py-2 flex items-center gap-4">
 <Button
 variant="ghost"
 size="sm"
 onClick={() => navigate(`/sections/${sectionId}`)}
 className="text-stone-600 hover:text-stone-900:text-stone-100 -ml-2"
 >
 ...
 : 'text-stone-400 hover:text-stone-600:text-stone-300 hover:bg-stone-100:bg-stone-800'
 }`}
 title="Mobile (30%)"
 >
 ...
 className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-stone-700:text-stone-200 transition-colors"
 >
 ...
 >
 <div className="w-1 h-16 rounded-full bg-stone-300 group-hover:bg-stone-400:bg-stone-500 transition-colors flex items-center justify-center">
 <GripVertical className="w-3 h-3 text-stone-500 opacity-0 group-hover:opacity-100 transition-opacity" strokeWidth={2} />
 </div>
 </div>
...
export function ScreenDesignFullscreen() {
 ...
 // Load product roadmap to get actual sections
 const productData = loadProductData()
 const sections = productData.roadmap?.sections || []

 // Create navigation items from actual sections (no Home button)
 const navigationItems = sections.map((section) => ({
 label: section.title,
 href: `/sections/${section.id}/screen-designs/${getDefaultScreenDesign(section.id)}/fullscreen`,
 ...
 <p className="text-stone-600">Screen design not found.</p>
 </div>
 )
 }
