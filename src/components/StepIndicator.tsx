...
export function StepIndicator({ step, status, children, isLast = false }: StepIndicatorProps) {
 ...
 {!isLast && (
 <div
 className="absolute left-[10px] top-[28px] w-[2px] h-[calc(100%+16px)] bg-stone-200"
 aria-hidden="true"
 />
 )}
...
function StepBadge({ step, status }: StepBadgeProps) {
 ...
 if (status === 'completed') {
 return (
 <div className={`${baseClasses} bg-stone-200 text-stone-500`}>
 ...
 if (status === 'current') {
 return (
 <div className={`${baseClasses} bg-stone-900 text-stone-100 shadow-sm`}>
